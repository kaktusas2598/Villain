...
namespace Villain {
    ...
    void Terrain::init(float scale, float texScale, std::vector<std::string> textureFilenames) {
        ...
        for (int i = 0; i < textureFilenames.size(); i++) {
            textures[i] = ResourceManager::Instance()->loadTexture(textureFilenames[i], textureFilenames[i], GL_REPEAT);
        }
        if (textureFilenames.size())
            terrainShader = Shader::createFromResource("multiTexTerrain");
        else
            terrainShader = Shader::createFromResource("basicTerrain");
    }
    ...
    void Terrain::render(RenderingEngine& renderingEngine, Camera* camera) {
        terrainShader->bind();
    ...
    void Terrain::loadHeightMap(const std::string& fileName) {
        int fileSize;
        heightMap = (float*)FileUtils::loadBinaryFile(fileName, fileSize);
